...
## Wiring the Admin Console to Supabase (do this once)

The admin dashboard needs to read every user's data, but normal accounts are locked to their own rows only (Row Level Security). This gives your own account a special exception without weakening security for anyone else.

### Step 1 — Run the migration
1. Go to your Supabase project → **SQL Editor** → **New query**
2. Open the file `supabase/admin-migration.sql` from this ZIP, copy all of it
3. Paste into the SQL Editor and click **Run**
4. You should see "Success. No rows returned"

### Step 2 — Sign up as your admin account (if you haven't already)
1. Go to your live site → Sign Up → create an account with the email you want to be admin (e.g., your own email)
2. Log in once with this account so it exists in the `profiles` table

### Step 3 — Flip the admin switch for your account
...
4. Verify: run `SELECT email, is_admin FROM profiles WHERE is_admin = TRUE;` — you should see your row

### Step 4 — Access the admin console
1. Log into the live site with your admin account (the one you just flagged)
2. Click "Admin Console" on the dashboard
3. Enter the PIN: `SD2025hub`
4. You should now see ALL users, ALL quiz results, ALL feedback

### If it shows "Only seeing your own account"
This means Step 3 didn't take effect. Check:
- Are you logged into the site with the exact email you ran the UPDATE for?
- Did the UPDATE query actually affect 1 row? (Case-sensitive email match)
- Did you run admin-migration.sql successfully first (Step 1)?

### Important
- The PIN alone does NOT grant data access — it's just a UI gate. The real security boundary is `is_admin = TRUE` in Supabase, checked by the database itself (Row Level Security).
- This means even if someone guesses the PIN, they only see their own data unless their account is flagged admin.
- To add a second admin later, just run the Step 3 UPDATE again for their email.
